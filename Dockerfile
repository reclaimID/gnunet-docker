...
RUN wget -q https://ftp.gnu.org/gnu/gnunet/gnurl-7_50_3.tar.bz2 -O gnurl.tar.bz2 && mkdir gnurl && tar xvf gnurl.tar.bz2 -C gnurl --strip-components 1 && cd gnurl && autoreconf -i && ./configure --prefix=/opt --disable-ntlm-wb --with-gnutls && make install && rm -rf /opt/gnurl*
