...
RUN wget -q https://ftp.gnu.org/gnu/gnunet/gnurl-7.62.0.tar.gz -O gnurl.tar.gz && tar xvf gnurl.tar.gz && cd gnurl-7.62.0/ && autoreconf -i && ./configure --prefix=/opt --disable-ntlm-wb --with-gnutls && make install && rm -rf /opt/gnurl*
